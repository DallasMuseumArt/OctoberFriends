...
81                     ->first();
82 
83                 $this->\hyperlink{classDMA_1_1Friends_1_1Wordpress_1_1User_aa2d2c2c9b82db76a0eab7af41629e011}{updateMetadata}($user, $id->ID);
84             \}
85         \});
86     \}
\end{DoxyCode}
...
92     \{
93         $metadata = $this->db
94             ->table(\textcolor{stringliteral}{'wp\_usermeta'})
95             ->where(\textcolor{stringliteral}{'user\_id'}, $wordpressId)
96             ->get();
97 
98         \textcolor{comment}{// Organize the metadata for mapping to user fields}
99         $data = [
100             \textcolor{stringliteral}{'home\_phone'}            => \textcolor{stringliteral}{''},
101             \textcolor{stringliteral}{'street\_address'}        => \textcolor{stringliteral}{''},
102             \textcolor{stringliteral}{'city'}                  => \textcolor{stringliteral}{''},
103             \textcolor{stringliteral}{'state'}                 => \textcolor{stringliteral}{''},
104             \textcolor{stringliteral}{'zip'}                   => \textcolor{stringliteral}{''},
105             \textcolor{stringliteral}{'first\_name'}            => \textcolor{stringliteral}{''},
106             \textcolor{stringliteral}{'last\_name'}             => \textcolor{stringliteral}{''},
107             \textcolor{stringliteral}{'\_badgeos\_points'}       => \textcolor{stringliteral}{''},
108             \textcolor{stringliteral}{'email\_optin'}           => \textcolor{keyword}{false},
109             \textcolor{stringliteral}{'current\_member'}        => \textcolor{keyword}{false},
110             \textcolor{stringliteral}{'current\_member\_number'} => \textcolor{stringliteral}{''},
111         ];
112 
113         \textcolor{keywordflow}{foreach}($metadata as $mdata) \{
114             $data[$mdata->meta\_key] = $mdata->meta\_value;
115         \}
116 
117         $user->phone            = $data[\textcolor{stringliteral}{'home\_phone'}];
118         $user->street\_addr      = $data[\textcolor{stringliteral}{'street\_address'}];
119         $user->city             = $data[\textcolor{stringliteral}{'city'}];
120         $user->zip              = $data[\textcolor{stringliteral}{'zip'}];
121         $user->points           = $data[\textcolor{stringliteral}{'\_badgeos\_points'}];
122 
123         \textcolor{comment}{// Populate state and country objects}
124         \textcolor{keywordflow}{if} (!empty($data[\textcolor{stringliteral}{'state'}])) \{
125             $state = State::where(\textcolor{stringliteral}{'code'}, strtoupper($data[\textcolor{stringliteral}{'state'}]))->first();
126             \textcolor{keywordflow}{if} (!$state) \{
127                 $state = State::where(\textcolor{stringliteral}{'name'}, $data[\textcolor{stringliteral}{'state'}])->first();
128             \}
129 
130             \textcolor{keywordflow}{if} ($state) \{
131                 $user->state()->associate($state);
132                 $user->country()->associate(Country::find($state->country\_id));
133             \}
134         \}
135 
136         $metadata                           = \textcolor{keyword}{new} Usermeta;
137         $metadata->first\_name               = $data[\textcolor{stringliteral}{'first\_name'}];
138         $metadata->last\_name                = $data[\textcolor{stringliteral}{'last\_name'}];
139         $metadata->email\_optin              = $data[\textcolor{stringliteral}{'email\_optin'}];
140         $metadata->current\_member           = $data[\textcolor{stringliteral}{'current\_member'}];
141         $metadata->current\_member\_number    = $data[\textcolor{stringliteral}{'current\_member\_number'}];
142 
143         \textcolor{keywordflow}{try} \{
144             $user->forceSave();
145             $user->metadata()->delete();
146             $user->metadata()->save($metadata);
147         \} \textcolor{keywordflow}{catch}(Exception $e) \{
148 
149         \}
150     \}
\end{DoxyCode}
