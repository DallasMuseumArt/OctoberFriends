...
162         \textcolor{keywordflow}{if} ($activity->activity\_lockout && $pivot = $user->activities()->where(\textcolor{stringliteral}{'activity\_id'}, $activity->id
      )->first()) \{
163             $time       = Carbon::now();
164             $lastTime   = $pivot->pivot->created\_at;
...
169 
170                 $message = self::convertToHoursMins($x, \textcolor{stringliteral}{'%d hours and %02d minutes'});
171 
172                 Session::put(\textcolor{stringliteral}{'activityError'}, Lang::get(\textcolor{stringliteral}{'dma.friends::lang.activities.lockout'}, [\textcolor{charliteral}{'x'} => 
      $message]));
173                 \textcolor{keywordflow}{return} \textcolor{keyword}{false};
174             \}
175         \}
176 
177         \textcolor{keywordflow}{switch} ($activity->time\_restriction) \{
178             \textcolor{keywordflow}{case} \hyperlink{classDMA_1_1Friends_1_1Models_1_1Activity_ab9dd8b18c4810beabdcf8e45039913c8}{Activity::TIME\_RESTRICT\_NONE}:
179                 \textcolor{keywordflow}{return} \textcolor{keyword}{true};
180             \textcolor{keywordflow}{case} \hyperlink{classDMA_1_1Friends_1_1Models_1_1Activity_ac78040e8784e02c2d1bcce5221ac6cb8}{Activity::TIME\_RESTRICT\_HOURS}:
181                 \textcolor{keywordflow}{if} ($activity->time\_restriction\_data) \{
182 
183                     $now    = Carbon::now();
184                     $start  = self::convertTime($activity->time\_restriction\_data[\textcolor{stringliteral}{'start\_time'}]);
185                     $end    = self::convertTime($activity->time\_restriction\_data[\textcolor{stringliteral}{'end\_time'}]);
186 
187                     $start\_time = Carbon::now();
188                     $start\_time->setTime($start[\textcolor{stringliteral}{'hour'}], $start[\textcolor{stringliteral}{'minutes'}]);
189                     $end\_time   = Carbon::now();
190                     $end\_time->setTime($end[\textcolor{stringliteral}{'hour'}], $start[\textcolor{stringliteral}{'minutes'}]);
191                     $day        = date(\textcolor{charliteral}{'w'});
192 
193                     \textcolor{comment}{// Sunday is on the end of the week and date sets sunday as 0}
194                     \textcolor{keywordflow}{if} ($day == 0) $day = 7;
195 
196                     \textcolor{keywordflow}{if} ($activity->time\_restriction\_data[\textcolor{stringliteral}{'days'}][$day] !== \textcolor{keyword}{false}
197                         && $now->gte($start\_time) && $now->lte($end\_time)) \{
198 
199                         \textcolor{keywordflow}{return} \textcolor{keyword}{true};
200                     \} \textcolor{keywordflow}{else} \{
201                         Session::put(\textcolor{stringliteral}{'activityError'}, Lang::get(\textcolor{stringliteral}{'dma.friends::lang.activities.notAvailable'}
      ));
202                     \}
203                 \}
204 
205                 \textcolor{keywordflow}{break};
206             \textcolor{keywordflow}{case} \hyperlink{classDMA_1_1Friends_1_1Models_1_1Activity_a71b85478f20cda144aeffe010364a0f7}{Activity::TIME\_RESTRICT\_DAYS}: 
207                 $now = \textcolor{keyword}{new} \hyperlink{namespaceDateTime}{DateTime}(\textcolor{stringliteral}{'now'});
208                 \textcolor{keywordflow}{if} ($now >= $activity->date\_begin 
209                     && $now <= $activity->date\_end) \{
210                     \textcolor{keywordflow}{return} \textcolor{keyword}{true};
211                 \} \textcolor{keywordflow}{else} \{
212                     Session::put(\textcolor{stringliteral}{'activityError'}, Lang::get(\textcolor{stringliteral}{'dma.friends::lang.activities.notAvailable'}));
213                 \}
214 
...
\begin{DoxyCode}
231     \{
232         list($hour, $minutes) = explode(\textcolor{stringliteral}{":"}, $timeString);
233         $meridiem = substr($minutes, 2, 3);
234         $minutes = substr($minutes, 0, 1);
235 
236 
237         \textcolor{keywordflow}{if} (strtolower($meridiem) == \textcolor{stringliteral}{'pm'} || ($hour == \textcolor{stringliteral}{'12'} && strtolower($meridiem) == \textcolor{stringliteral}{'am'})) \{
238             $hour += 12;
239         \}
240 
241         \textcolor{keywordflow}{return} [
242             \textcolor{stringliteral}{'hour'}      => $hour,
243             \textcolor{stringliteral}{'minutes'}   => $minutes,
244         ];
245     \}
\end{DoxyCode}
...
127 
128                 Event::fire(\textcolor{stringliteral}{'dma.friends.activity.completed'}, [ $activity, $user ]); 
129 
