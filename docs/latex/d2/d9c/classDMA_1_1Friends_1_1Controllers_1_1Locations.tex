...
59                 $usermeta = Usermeta::with(\textcolor{stringliteral}{'user'})->where(\textcolor{stringliteral}{'current\_member\_number'}, \textcolor{charliteral}{'='}, $barcodeId)->first(
      );
60                 \textcolor{keywordflow}{if} (isset($usermeta->user) && !empty($usermeta->user)) \{
61                     $user = $usermeta->user;
62                 \}
63             \}
64 
65             \textcolor{keywordflow}{if} (!$user) \{
66                 Log::debug(\textcolor{stringliteral}{"Failed to login user"}, [\textcolor{stringliteral}{'barcodeId'} => $barcodeId, \textcolor{stringliteral}{'user'} => $user]);
67                 \textcolor{comment}{// The user does not exist, so flash an error}
68                 Flash::error(Lang::get(\textcolor{stringliteral}{'dma.friends::lang.app.loginFailed'}));
69             \} \textcolor{keywordflow}{else} \{
70                 Log::debug(\textcolor{stringliteral}{"Logged in user"}, [\textcolor{stringliteral}{'barcodeId'} => $barcodeId, \textcolor{stringliteral}{'user'} => $user]);
71                 \textcolor{comment}{//The user exists so log them in}
72                 Auth::login($user);
73                 Event::fire(\textcolor{stringliteral}{'auth.login'}, [$user]);
74             \}
75 
76         \}
77 
78         \textcolor{keywordflow}{return} Redirect::to(\textcolor{charliteral}{'/'});
79     \}
\end{DoxyCode}
