...
\begin{DoxyCode}
333     \{
334         $theme = Theme::getActiveTheme();
335         \textcolor{keywordflow}{return} $theme->getPath();
336     \}
\end{DoxyCode}
...
\begin{DoxyCode}
323     \{
324         \textcolor{keywordflow}{return} self::getThemeDir() . \textcolor{stringliteral}{'/partials/'};
325     \}
\end{DoxyCode}
...
\begin{DoxyCode}
299     \{   
300         $partialsDir = self::getThemePartialsDir();
301         $partialPath = $partialsDir . $partial . \textcolor{stringliteral}{'.htm'};
302 
303         \textcolor{keywordflow}{if} (!File::isFile($partialPath)) \{
304             $partialPath = $this->getViewPath($partial) . \textcolor{stringliteral}{'.htm'};
305         \}
306 
307         \textcolor{keywordflow}{if} (!File::isFile($partialPath)) \{
308             \textcolor{keywordflow}{if} ($throwException)
309                 \textcolor{keywordflow}{throw} \textcolor{keyword}{new} \hyperlink{namespaceSystemException}{SystemException}(Lang::get(\textcolor{stringliteral}{'backend::lang.partial.not\_found'}, [\textcolor{stringliteral}{'
      name'} => $partialPath]));
310             \textcolor{keywordflow}{else}
311                 \textcolor{keywordflow}{return} \textcolor{keyword}{false};
312         \}   
313 
314         \textcolor{keywordflow}{return} $this->makeFileContents($partialPath, $params);
315     \}   
\end{DoxyCode}
...
203         $userData = [
204             \textcolor{stringliteral}{'username'}              => md5($data[\textcolor{stringliteral}{'email'}]), \textcolor{comment}{//$data['username'],}
...
216 
217         \textcolor{comment}{// Save user metadata}
218         $usermeta = \textcolor{keyword}{new} Usermeta;
219         $usermeta->first\_name       = $data[\textcolor{stringliteral}{'first\_name'}];
220         $usermeta->last\_name        = $data[\textcolor{stringliteral}{'last\_name'}];
221         $usermeta->gender           = $data[\textcolor{stringliteral}{'gender'}];
222         $usermeta->birth\_date       = $data[\textcolor{stringliteral}{'birth\_date'}];
223         $usermeta->race             = $data[\textcolor{stringliteral}{'race'}];
224         $usermeta->household\_income = $data[\textcolor{stringliteral}{'household\_income'}];
225         $usermeta->household\_size   = $data[\textcolor{stringliteral}{'household\_size'}];
226         $usermeta->education        = $data[\textcolor{stringliteral}{'education'}];
227         $usermeta->email\_optin      = isset($data[\textcolor{stringliteral}{'email\_optin'}]) ? $data[\textcolor{stringliteral}{'email\_optin'}] : \textcolor{keyword}{false};
228 
229         $user->metadata()->save($usermeta);
230 
231         UserExtend::uploadAvatar($user, $data[\textcolor{stringliteral}{'avatar'}]);
232 
233         \textcolor{comment}{/*}
234 \textcolor{comment}{         * Activation is by the user, send the email}
235 \textcolor{comment}{         */}
236         \textcolor{keywordflow}{if} ($userActivation) \{
237             $this->sendActivationEmail($user);
238         \}
239 
240         \textcolor{comment}{/*}
241 \textcolor{comment}{         * Automatically activated or not required, log the user in}
242 \textcolor{comment}{         */}
243         \textcolor{keywordflow}{if} ($automaticActivation || !$requireActivation) \{
244             Auth::login($user);
245         \}
246 
247         \textcolor{comment}{/*}
248 \textcolor{comment}{         * Fire event that user has registered}
249 \textcolor{comment}{         */}
250         Event::fire(\textcolor{stringliteral}{'auth.registered'}, [$user]);
251 
252         \textcolor{comment}{/*}
253 \textcolor{comment}{         * Redirect to the intended page after successful sign in}
254 \textcolor{comment}{         */}
255         $redirectUrl = $this->pageUrl($this->property(\textcolor{stringliteral}{'redirect'}));
256 
257         \textcolor{keywordflow}{if} ($redirectUrl = post(\textcolor{stringliteral}{'redirect'}, $redirectUrl))
258             \textcolor{keywordflow}{return} Redirect::intended($redirectUrl);
259     \}
\end{DoxyCode}
