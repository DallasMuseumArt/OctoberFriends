...
\begin{DoxyCode}
92     \{
93         $user   = Auth::getUser();
94         $count  = $user->badges()->where(\textcolor{stringliteral}{'badge\_id'}, \textcolor{charliteral}{'='}, $this->\textcolor{keywordtype}{id})->count();
95 
96         \textcolor{keywordflow}{return} $count;
97     \}
\end{DoxyCode}
...
\begin{DoxyCode}
105     \{
106         $user = Auth::getUser();
107         $ids = [];
108         $stepCount = 0;
109 
110         \textcolor{keywordflow}{foreach}($this->steps as $step) \{
111             $ids[] = $step->id;
112         \}
113 
114         \textcolor{keywordflow}{if} ($ids && $this->userCount) \{
115             $stepCount = DB::table(\textcolor{stringliteral}{'dma\_friends\_step\_user'})
116                 ->whereIn(\textcolor{stringliteral}{'step\_id'}, $ids)
117                 ->where(\textcolor{stringliteral}{'user\_id'}, $user->id)
118                 ->count();
119 
120             $stepCount = $stepCount / $this->userCount;
121         \}
122         
123         \textcolor{keywordflow}{if} ($stepCount)
124             \textcolor{keywordflow}{return} (count($this->steps) / $stepCount) * 100;
125         
