...
\begin{DoxyCode}
93     \{
94         \textcolor{keywordflow}{if} (!isset($this->pivot->created\_at)) \textcolor{keywordflow}{return} null;
95 
96         $timeAgo = \textcolor{keyword}{new} TimeAgo;
97         \textcolor{keywordflow}{return} $timeAgo->get($this->pivot->created\_at);
98     \}
\end{DoxyCode}
...
\begin{DoxyCode}
106     \{
107         $user   = Auth::getUser();
108         $count  = $user->badges()->where(\textcolor{stringliteral}{'badge\_id'}, \textcolor{charliteral}{'='}, $this->\textcolor{keywordtype}{id})->count();
109 
...
\begin{DoxyCode}
119     \{
120         $user = Auth::getUser();
121         $ids = [];
122         $stepCount = 0;
123 
124         \textcolor{keywordflow}{foreach}($this->steps as $step) \{
125             $ids[] = $step->id;
126         \}
127 
128         \textcolor{keywordflow}{if} ($ids && $this->userCount) \{
129             $stepCount = DB::table(\textcolor{stringliteral}{'dma\_friends\_step\_user'})
130                 ->whereIn(\textcolor{stringliteral}{'step\_id'}, $ids)
131                 ->where(\textcolor{stringliteral}{'user\_id'}, $user->id)
132                 ->count();
133 
134             $stepCount = $stepCount / $this->userCount;
135         \}
136         
137         \textcolor{keywordflow}{if} ($stepCount)
138             \textcolor{keywordflow}{return} (count($this->steps) / $stepCount) * 100;
139         
...
\begin{DoxyCode}
81     \{
82         \textcolor{keywordflow}{return} $query->where(\textcolor{stringliteral}{'is\_published'}, \textcolor{keyword}{true})
83                      ->where(\textcolor{stringliteral}{'is\_archived'},  \textcolor{keyword}{false});
84     \}
\end{DoxyCode}
