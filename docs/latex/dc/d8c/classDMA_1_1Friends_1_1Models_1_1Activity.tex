...
\begin{DoxyCode}
170     \{
171         \textcolor{keywordflow}{if} (!isset($this->pivot->created\_at)) \textcolor{keywordflow}{return} null;
172 
173         $timeAgo = \textcolor{keyword}{new} TimeAgo;
174         \textcolor{keywordflow}{return} $timeAgo->get($this->pivot->created\_at);
175     \}
\end{DoxyCode}
...
\begin{DoxyCode}
97     \{
98         \textcolor{keywordflow}{return} unserialize($value);
99     \}
\end{DoxyCode}
...
\begin{DoxyCode}
140                                                          \{
141         \textcolor{keywordflow}{if} (is\_array($categories)) \{
142             \textcolor{comment}{// Get first category in list to begin query with whereHas}
143             $firstcategory = array\_shift($categories);
144             $query = $query->whereHas(\textcolor{stringliteral}{'categories'}, \textcolor{keyword}{function}($q) use ($firstcategory) \{
145                 $q->where(\textcolor{stringliteral}{'name'}, $firstcategory);
146             \});
147 
148             \textcolor{comment}{// if any categories left in list, iterate over for orWhereHas}
149             \textcolor{keywordflow}{foreach} ($categories as $category) \{
150                 $query = $query->orWhereHas(\textcolor{stringliteral}{'categories'}, \textcolor{keyword}{function}($q) use ($category) \{
151                     $q->where(\textcolor{stringliteral}{'name'}, $category);
152                 \});
153             \}
154             
155             \textcolor{keywordflow}{return} $query;
156         \}
157         \textcolor{keywordflow}{else} \{
158             \textcolor{comment}{// if $categories is a string, only one category to filter against}
159             \textcolor{keywordflow}{return} $query->whereHas(\textcolor{stringliteral}{'categories'}, \textcolor{keyword}{function}($q) use ($categories) \{
160                 $q->where(\textcolor{stringliteral}{'name'}, $categories);
161             \});
162         \}
163     \}
\end{DoxyCode}
...
\begin{DoxyCode}
123     \{
124         \textcolor{keywordflow}{return} $query->where(\textcolor{stringliteral}{'activity\_type'}, $type)
125             ->isActive();
126     \}
\end{DoxyCode}
...
\begin{DoxyCode}
114     \{
115         \textcolor{keywordflow}{return} $query->where(\textcolor{stringliteral}{'activity\_code'}, $code)
116             ->isActive();
117     \}
\end{DoxyCode}
...
\begin{DoxyCode}
132     \{   
133         \textcolor{keywordflow}{return} $query->where(\textcolor{stringliteral}{'wordpress\_id'}, $id);
134     \}  
\end{DoxyCode}
...
\begin{DoxyCode}
105     \{
106         \textcolor{keywordflow}{return} $query->where(\textcolor{stringliteral}{'is\_published'}, \textcolor{charliteral}{'='}, 1)
107             ->where(\textcolor{stringliteral}{'is\_archived'}, \textcolor{stringliteral}{'<>'}, 1);
108     \}
\end{DoxyCode}
...
\begin{DoxyCode}
85     \{
86         \textcolor{keywordflow}{if} (is\_array($value)) \{
87             $value = serialize($value);
88         \}
89 
90         $this->attributes[\textcolor{stringliteral}{'time\_restriction\_data'}] = $value;
91     \}
\end{DoxyCode}
